...
// 상속
// 기존에 정의되어 있는 (상위)클래스의 모든 멤버 변수와 멤버 함수를 물려 받아, 하위 클래스가 사욜할 수 있는 것입니다.
...
int main()
{
    ...
    */
    ...
    // 특정 시점에 배열의 크기를 100개 사용해야 되지만, 그 특정 시점을 넘어가면 더이상 사용할 필요가 없을 때

    // 동적 할당은 실행 시간에 메모리 공간 확보하고 줄이고 할 수 있습니다.
    // 리스트

    // 생성자
    // 클래스의 객체가 생성되었을 때 객체를 초기화하는 목적으로 자동으로 호출되는 함수입니다.
