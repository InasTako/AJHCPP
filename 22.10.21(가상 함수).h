...
public:
    ...
    // -> 가상 함수는 상위 클래스와 하위 클래스의 형태가 동일해야 합니다.
...
int main()
{
    ...
    // 컴파일 시점에 주소값이랑 10이라는 값을 확정하는 단계
    // int a = 10;

    // 정적 바인딩
    // 컴파일 시점에 이루어지는 바인딩입니다.
    // 컴파일이 끝나면 결정된 속성들은 변경이 불가능 합니다.

    // 동적 바인딩
    // 실행시간에 이루어지는 바인딩입니다.
    // 실행시간에 필요한 객체의 함수를 호출할 수 있으며, 유연성을 가질수 있습니다.
