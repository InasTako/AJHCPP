...
{
    // 접근 지정자
    // 클래스 내부에서 접근 가능한 멤버(변수, 함수)와 클래스 외부에서 
    // 접근이 허용되지 않는 멤버(변수, 함수)를 구분하는 지정자입니다.

    // public : 클래스 내부와 자기가 상속하고 있는 클래스 그리고 클래스 외부에서 접근을 허용할 수 있는 지정자입니다.
    // private : 클래스 내부에서만 접근이 허용되고, 자기가 상속하는 클래스와 외부에서 접근할 수 없는 지정자입니다.
    // protected : 클래스 내부와 자기가 상속하는 클래스까지만 접근을 허용되고 클래스 외부에서는 접근을 할 수 없는 지정자입니다.
...
public:
    int health = 100;
    float size = 3.65;
    std::string name = "Ninomae Inanis";
...
int main()
{
    ...
    std::cout << "두 번째 안녕하세요." << std::endl;
    */

    // vool = 참과 거짓을 나타내는 자료형입니다.
    // bool value = true;
    ...
    */
    ...
    std::cout << Ina.health << " " << Ina.size << " " << Ina.name << std::endl;
